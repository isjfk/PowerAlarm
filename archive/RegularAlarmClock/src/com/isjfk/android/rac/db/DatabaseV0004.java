 ...
 */
public class DatabaseV0004 implements Database {
    ...
    public void upgrade(Context context, SQLiteDatabase db) {
        db.execSQL("ALTER TABLE schedule ADD COLUMN orgDateTime");

        SparseArray<String> dateTimes = new SparseArray<String>();
        Cursor cursor = db.query("schedule", new String[] {"_id", "dateTime"}, null, null, null, null, null);
        if (cursor != null) {
            try {
                while (cursor.moveToNext()) {
                    dateTimes.put(cursor.getInt(0), cursor.getString(1));
                }
            } finally {
                cursor.close();
            }
        ...
        for (int i = 0; i < dateTimes.size(); i++) {
            int key = dateTimes.keyAt(i);
            String value = dateTimes.valueAt(i);
            db.execSQL("UPDATE schedule SET orgDateTime=\"" + value + "\" WHERE _id=" + key);
        }
    }
